...
| 테스트 | 위치 | 잡아주는 회귀 |
|---|---|---|
| `TestRegister_All8ToolsListed` | `internal/mcp/register_test.go` | tool 누락 / 오타 / 이름 변경 (예: `rune_caputre`) |
| `TestRegister_InputSchemaInferred` | 동 | domain struct 가 schema 화 불가능한 형태로 변경 (`map[interface{}]any` 등) |
| `TestRegister_StubReturnsIsError` (4 subtest) | 동 | stub 응답 포맷 변경, IsError 누락, "not yet implemented" 문구 깨짐 |
...
| `tool[N]: got "X", want "Y"` | `expectedTools` 알파벳 순서 미준수 또는 이름 변경 |
| `InputSchema is nil` | input struct 가 schema inference 안 되는 타입 |
