...
| D10 | near_duplicate 시 `similar_to` 값 형식 | 📦 Archived (2026-04-22) — Python parity 우선 (`novelty.related[]` 사용, `similar_to` 필드 drop) | — |
...
**상태**: 📦 Archived (2026-04-22) — Python parity 우선으로 `similar_to` 필드 자체 drop  
**이전 결론**: record_id 문자열 (best-effort lookup) + shard/row 좌표 degrade  
**현재 상태**: **`similar_to` 필드 포팅하지 않음**. Python 응답에는 이 필드가 없고, 중복 정보는 `novelty.related[]` 배열의 `id`/`title`/`similarity`로 이미 제공됨. 에이전트는 `response.novelty.related[0].id`로 가장 유사한 record에 접근 가능. 이 결정 항목은 히스토리 보존용.
