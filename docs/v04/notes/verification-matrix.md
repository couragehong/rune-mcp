...
### C.2 ✅ envector 11 연결 에러 패턴 — **해소됨** (2026-04-22)

- **Python**: `envector_sdk.py:L89-101` 11개 string pattern (`CONNECTION_ERROR_PATTERNS`)
- **Go**: `spec/components/envector.md` "Python 대비 (의도적 차이)" 섹션 신설 — typed error + gRPC status code 매핑 전략 명시 + 11 패턴 미포팅 이유 기록
- **판정**: **기능 동등, 구현 전략 차이 (의도적)**. Go SDK가 structured error를 이미 노출하므로 string matching 불필요.
...
### C.10 ✅ Q1-Q9 상태 — **정비 완료** (2026-04-22)

| Q | 상태 | 비고 |
|---|---|---|
| Q1 AES envelope MAC | 🔵 Deferred | Post-MVP (D1) |
| Q2 embedder 엔진 | 📦 Archived | embedder 프로젝트 이관 |
| Q3 Multi-MCP ActivateKeys race | ✅ Decided (b) | Python 실측 기반, 재평가 트리거 기록 |
| Q4 envector-go PR | 🟡 External pending | 외부 머지 대기, 구현 병렬 가능 |
| Q5 설치 순서 | 📦 Archived | embedder 프로젝트 이관 |
| Q6 버전 호환 | 📦 Archived | embedder 프로젝트 이관 |
| Q7 socket 보안 | 📦 Archived | embedder 프로젝트 이관 |
| Q8 capture_log 저장 | ✅ Decided (a) | flock + Mutex, spec에 반영 완료 |
| Q9 Vault 오타 UX | ✅ Decided | tool_vault_status 스펙에 반영 완료 |

**실제 남은 open items: Q1 (Deferred) · Q4 (external PR 대기) 2건.**  
**구현 blocking: 0건.**
...
7. ~~**[C.2] envector error 매핑 방식 설명 추가**~~ ✅ 해소 (2026-04-22) — `spec/components/envector.md` Python 대비 섹션 신설
8. ~~**[C.10] open-questions.md Q1-Q9 전체 재검토**~~ ✅ 해소 (2026-04-22) — Q3/Q8/Q9 Decided, Q5-Q7 Archived, Q1/Q4만 active
